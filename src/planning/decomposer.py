    ...
    async def decompose(
        ...
        try:
            ...
            for i, item in enumerate(data.get("subtasks", [])):
                ...
                subtasks[i].depends_on = [uuid_map[idx] for idx in dep_indices if idx in uuid_map]
...
class TaskDecomposer:
    ...
    def build_execution_order(self, subtasks: list[SubTask]) -> list[list[SubTask]]:
        ...
        while remaining:
            ...
            ready = [id_to_task[tid] for tid in remaining if id_to_task[tid].can_execute(completed)]
